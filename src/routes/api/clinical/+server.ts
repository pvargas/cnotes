...
export async function GET({ url } : any) {
  const result = await getClinicalData(url.searchParams);
	return json({result: result}, { status: 200 });
}

function parseClinicalData(data: any) {
  let encoded = null;
  let decoded = null;
  if (data) {
...
async function getClinicalData(params: any) {
  ...
  const endDate = params.get("startDate");
  ...
  rawNotes = parseClinicalData(testDoc)
  
  // // uncomment use live FHIR server, if available.
  // // may not work
